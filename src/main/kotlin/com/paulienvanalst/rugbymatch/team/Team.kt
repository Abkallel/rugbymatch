...
data class Team (val players: List<Player>, val name: TeamName) {
    ...
    val hasEnoughStartingPlayers : Boolean
        get() = players.filter { it.backNumber in 1..15 }.size>=15
    ...
    fun replacingCaptainBackNumber(): Int? {
        ...
        return scrumhalf()?.backNumber ?: this.players.first().backNumber

    }

    private fun scrumhalf(): Player? {
        return this.players.find { it.position == Position.SCRUM_HALF }
    }
